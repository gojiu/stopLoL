...
void terminateLoL()
{
	...
	if (hProcessus == INVALID_HANDLE_VALUE && Process32First(hProcessus, &processDetail)) // if there are still processes and if Process32First function succeed
	{
		while (Process32Next(hProcessus, &processDetail)) // while there are still processes in the snapshot
		{
			if (_tcscmp(processDetail.szExeFile, _T("League of Legends.exe")) == 0) // function to compare 2 wchar_t variables, here we check if League of Legends.exe is in the running processes
			{
